    ...
    function process(JGO, div) {
      ...
      for (c.j = TL.j; c.j <= BR.j; ++c.j) {
        for (c.i = TL.i; c.i <= BR.i; ++c.i) {
          ...
          //if (elem.number) jboard.setNumber(c, elem.number);
        }
      }
  ...
  2: [function (require, module, exports) {
    ...
    var Board = function (width, height) {
      ...
      for (var i = 0; i < this.width; ++i) {
        var stoneArr = [], markArr = [], numberArr = [];

        for (var j = 0; j < this.height; ++j) {
          stoneArr.push(C.CLEAR);
          markArr.push(C.MARK.NONE);
          numberArr.push(C.MARK.NONE);
        }
        this.number.push(numberArr);
    ...
    Board.prototype.each = function (func, i1, j1, i2, j2) {
      ...
      for (c.j = j1; c.j <= j2; c.j++)
        for (c.i = i1; c.i <= i2; c.i++)
          func(c.copy(), this.stones[c.i][c.j], this.marks[c.i][c.j], this.number[c.i][c.j]);
    };
    ...
    /**
     * Set the intersection number at given coordinate(s).
     *
     * @param {Object} c A Coordinate or Array of them.
     * @param {Object} n New number, e.g. 
     */
    Board.prototype.setNumber = function (c, n) {
      if (c instanceof Coordinate) {
        var old = this.number[c.i][c.j];

        if (old == n) return; // no change

        this.number[c.i][c.j] = n;

        var ev = {
          type: 'num', coordinate: c, board: this,
          oldVal: old, newVal: n
        };
        this.listeners.forEach(function (l) { l(ev); });
      } else if (c instanceof Array) {
        for (var i = 0, len = c.length; i < len; ++i)
          this.setNumber(c[i], n); // use ourself to avoid duplicate code
      }
    };
 ...
 * @returns {Object} Mark or array of marks.
 */
    Board.prototype.getNumber = function (c) {
      var ret;
  ...
  3: [function (require, module, exports) {
    ...
    Canvas.prototype.draw = function (jboard, i1, j1, i2, j2) {
      ...
      // Stones and marks
      jboard.each(function (c, type, mark, number) {
        var ox = (this.getX(c.i - this.opt.view.xOffset));
        var oy = (this.getY(c.j - this.opt.view.yOffset));
        var markColor;
        ...
        switch (type) {
          ...
          case C.DIM_BLACK:
            ...
            this.stones.drawMark(this.ctx, number, ox, oy);
            break;
          case C.WHITE:
          case C.DIM_WHITE:
  ...
  }, { "./auto": 1, "./board": 2, "./canvas": 3, "./constants": 4, "./coordinate": 5, "./node": 7, "./notifier": 8, "./record": 9, "./setup": 10, "./sgf": 11, "./stones": 12, "./util": 13 }],
  7: [function (require, module, exports) {
     ...
     * @param {int} val Mark.
     */
    Node.prototype.setMark = function (c, val, num = 0) {
      if (c instanceof Array) {
        for (var i = 0, len = c.length; i < len; ++i)
 ...
 * @param {int} val Mark.
 */
    Node.prototype.setNumber = function (c, val) {
      if (c instanceof Array) {
        for (var i = 0, len = c.length; i < len; ++i)
          this.setNumber(c[i], val); // avoid repeating ourselves
        return;
      }

      // Store both change and previous value to enable reversion
      this.changes.push({ c: c.copy(), num: val, old: this.jboard.getNumber(c) });
      this.jboard.setNumber(c, val);
    };
    ...
    Node.prototype.apply = function () {
      for (var i = 0; i < this.changes.length; i++) {
        ...
        if ('type' in item) {
          ...
          this.jboard.setNumber(item.c, item.num);
        } else {
    ...
    Node.prototype.revert = function () {
      for (var i = this.changes.length - 1; i >= 0; i--) {
        ...
        if ('type' in item) {
          ...
          this.jboard.setNumber(item.c, item.old);
        } else {
  ...
  9: [function (require, module, exports) {
     ...
     * @returns {Node} New, current node.
     */
    Record.prototype.createVarNode = function (clearParentMarks, options) {
      var node = new varNode(this.jboard, this.current, options);

      if (clearParentMarks)
        node.clearParentMarks();

      if (this.varRoot === null)
        this.varRoot = node;
  ...
  11: [function (require, module, exports) {
    ...
    function sgfMove(node, name, values, moveMarks, i) {
      ...
      node.info.coord = `${name}[${SGFLetters[coord.i]}${SGFLetters[coord.j]}]`;
      node.info.playeralphabat = name
      node.info.moveNum = i
      ...
      for (var i = 0; i < gameTree.sequence.length; i++) {
        ...
        for (var key in node) {
          if (node.hasOwnProperty(key)) {
            ...
            if (!SGFProperties[key](jnode, key, node[key], moveMarks, i)) {
  ...
  12: [function (require, module, exports) {
    ...
    Stones.prototype.drawMark = function (ctx, mark, ox, oy, num = 0) {
      switch (mark) {
        case C.MARK.SQUARE:
          ctx.beginPath();
